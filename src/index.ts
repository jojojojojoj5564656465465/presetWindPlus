			...
			rules: [
				[
					...
					(match) => {
						...
						const basis = v.parse(v.string("basis is not a string flex-basis rules"), match.groups?.basis);
				...
				[
					...
					new RegExp(`^(?<direction>p|m|inset)-${regexUnit}-?${regexUnit}?-?${regexUnit}?-?${regexUnit}?$`),
					([, ...match]) => {
						const [direction, ...units] = match;

						try {
							const arrMatch = v.parse(fromMatchRemoveDuplicate(4, false), matchUnitsNonProcessed(units));
